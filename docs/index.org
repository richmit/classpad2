...
:END:
 ...
 - *=jacobian(expression_in_x_y_and_z)=*: Compute the Jacobin matrix of an expression \\
   ...
   #+BEGIN_CENTER
   file:pics/jacobian-input.png
   #+END_CENTER
 # - *=gradient(expression_in_x_y_and_z)=*: Compute the Gradiant vector of an expression (always a row vector).
 #   #+BEGIN_CENTER
 #   file:pics/jacobian-gradient.png
 #   #+END_CENTER
 # - *=hessian(expression_in_x_y_and_z)=*: Compute the Hessian matrix of a scalar valued, bivariate/trivariate expression.
 #   #+BEGIN_CENTER
 #   file:pics/gradient-hessian.png
 #   #+END_CENTER
 - *=mkDmat(diag_as_list_matrix_or_vector)=*: Make a diagonal matrix
 - *=mkMat(expression_in_x_and_y, number_of_rows, number_of_columns)=*: Make a matrix from an index expression
 # - *=newtonC(expression_in_x, initial_guess, epsilon)=*: Search for a root of a univariate expression using Newton's method with complex arithmetic. \\
 #   *NOTE:* This function will search for a root until it finds a zero or until the user cancels it. 
 #   #+BEGIN_CENTER
 #   file:pics/newtonC.png
 #   #+END_CENTER
 # - *=newtonCi(expression_in_x, initial_guess, epsilon)=*: Just like *=newtonC=* except pauses every 10 iterations and queries the user to continue.
 # - *=newtonR2(expression_in_x, initial_guess, epsilon)=*: Search for a root system of two bivariate expressions using Newton's method with real arithmetic.  \\
 #   This program accepts a two component, bivariate expression as it's first argument, and a point in \( \mathbb{R}^2 \) as it's second argument.  Each 
 #   argument may be provided as a list of two elements, a two element row vector, or a two element column vector.  Both must be of the same form -- i.e. if
 #   one is a list, then they both must be lists.  No mater the input form, the result will always be a two element column vector. \\
 #   *NOTE:* This function will search for a root until it finds a zero or until the user cancels it. 
 #   #+BEGIN_CENTER
 #   file:pics/newtonR2.png
 #   #+END_CENTER
 # - *=newtonR3(expression_in_x, initial_guess, epsilon)=*: Just like *=newtonR2=*, but for systems of 3 equations.
...
to deal with the grumpy user interface. =:)=
 ...
 # - *=subsCVec(expression, column_vector)=*: Substitute the first element of column_vector into the expression as x, the second as y, and the third as z.the x into expression. 
...
magick pics/jacobian-input.bmp    -gravity South -chop 0x185 pics/jacobian-input.png
magick pics/jacobian-gradient.bmp -gravity South -chop 0x90  pics/jacobian-gradient.png
magick pics/gradient-hessian.bmp  -gravity South -chop 0x55  pics/gradient-hessian.png
magick pics/newtonC.bmp           -gravity South -chop 0x152 pics/newtonC.png
magick pics/newtonR2.bmp          -gravity South -chop 0x123 pics/newtonR2.png

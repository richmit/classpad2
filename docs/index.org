...
:END:
 ...
 - *=newtonC(expression_in_x, initial_guess, epsilon)=*: Solve complex equations with Newton's method.  Query user every 10 iterations to continue.
 - *=newtonCC(expression_in_x, initial_guess, epsilon)=*: Solve complex equations with Newton's method.  Runs forever trying to find a root.
